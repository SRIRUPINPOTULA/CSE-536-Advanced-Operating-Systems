...
growproc(int n)
{
  ...
  sz = p->sz;
  int total_pages = n/4096;
  sz = p->sz;
  if(p->ondemand==true)
  {
    track_heap(p,sz, total_pages);
    print_skip_heap_region(p->name, sz, total_pages);
    p->sz=sz+n;
    return 0;
  }
  if(n > 0){
...
fork(int cow_enabled)
{
  ...
  if(cow_enabled==1)
  {
    ...
    //printf("****************************");
    //printf("The value is: %d", p->sz);
    //printf("THe p->pagetable: %d", p->pagetable);
    //printf("THe p->pagetable: %d", np->pagetable);
